...
export function migrateCareerState(value: unknown): CareerState | null {
  ...
  const migrated: CareerState = {
    ...
    inventory: source.inventory && Array.isArray(source.inventory.upgrades) ? { ...source.inventory, properties: source.inventory.properties ?? [], investments: source.inventory.investments ?? [], purchaseHistory: source.inventory.purchaseHistory ?? [] } : { upgrades: [], properties: [], investments: [], purchaseHistory: [] },
