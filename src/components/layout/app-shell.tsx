...
const symbols = {
  dashboard: { ios: 'square.grid.2x2.fill', android: 'dashboard', web: 'dashboard' }, profile: { ios: 'person.crop.circle.fill', android: 'account_circle', web: 'account_circle' }, calendar: { ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }, decision: { ios: 'arrow.triangle.branch', android: 'alt_route', web: 'alt_route' }, tournament: { ios: 'trophy.fill', android: 'emoji_events', web: 'emoji_events' }, match: { ios: 'scope', android: 'sports_esports', web: 'sports_esports' }, statistics: { ios: 'chart.bar.fill', android: 'query_stats', web: 'query_stats' }, rankings: { ios: 'list.number', android: 'leaderboard', web: 'leaderboard' }, major: { ios: 'shield.fill', android: 'shield', web: 'shield' }, team: { ios: 'shield.lefthalf.filled', android: 'shield_person', web: 'shield_person' }, roster: { ios: 'person.3.fill', android: 'groups', web: 'groups' }, market: { ios: 'arrow.left.arrow.right', android: 'swap_horiz', web: 'swap_horiz' }, contract: { ios: 'doc.text.fill', android: 'description', web: 'description' }, training: { ios: 'figure.strengthtraining.traditional', android: 'fitness_center', web: 'fitness_center' }, health: { ios: 'heart.text.square.fill', android: 'health_and_safety', web: 'health_and_safety' }, analytics: { ios: 'chart.xyaxis.line', android: 'analytics', web: 'analytics' }, timeline: { ios: 'clock.arrow.trianglehead.counterclockwise.rotate.90', android: 'timeline', web: 'timeline' }, awards: { ios: 'medal.fill', android: 'military_tech', web: 'military_tech' }, trophies: { ios: 'trophy.fill', android: 'workspace_premium', web: 'workspace_premium' }, records: { ios: 'rosette', android: 'hotel_class', web: 'hotel_class' }, lifestyle: { ios: 'bag.fill', android: 'shopping_bag', web: 'shopping_bag' }, inventory: { ios: 'shippingbox.fill', android: 'inventory_2', web: 'inventory_2' }, investments: { ios: 'building.columns.fill', android: 'account_balance', web: 'account_balance' }, settings: { ios: 'gearshape.fill', android: 'settings', web: 'settings' },
} satisfies Record<string, SymbolName>;

const groups: { label: string; items: { id: ScreenId; label: string; icon: keyof typeof symbols }[] }[] = [
  { label: 'Carrera', items: [{ id: 'dashboard', label: 'Dashboard', icon: 'dashboard' }, { id: 'profile', label: 'Perfil', icon: 'profile' }, { id: 'calendar', label: 'Calendario', icon: 'calendar' }, { id: 'decision', label: 'Decisiones', icon: 'decision' }] },
  { label: 'Competición', items: [{ id: 'tournament', label: 'Torneos', icon: 'tournament' }, { id: 'match', label: 'Partidos', icon: 'match' }, { id: 'statistics', label: 'Estadísticas', icon: 'statistics' }, { id: 'rankings', label: 'Top 100 mundial', icon: 'rankings' }, { id: 'major-hub', label: 'Major Hub', icon: 'major' }] },
  { label: 'Organización', items: [{ id: 'team', label: 'Equipo', icon: 'team' }, { id: 'roster', label: 'Roster', icon: 'roster' }, { id: 'market', label: 'Mercado', icon: 'market' }, { id: 'contract', label: 'Contrato', icon: 'contract' }] },
  { label: 'Desarrollo', items: [{ id: 'training', label: 'Entrenamiento', icon: 'training' }, { id: 'health', label: 'Salud y fatiga', icon: 'health' }, { id: 'analytics', label: 'Analytics', icon: 'analytics' }] },
  { label: 'Legado', items: [{ id: 'timeline', label: 'Timeline', icon: 'timeline' }, { id: 'awards', label: 'Premios', icon: 'awards' }, { id: 'trophy-room', label: 'Trofeos', icon: 'trophies' }, { id: 'records', label: 'Récords', icon: 'records' }] },
  { label: 'Finanzas', items: [{ id: 'lifestyle', label: 'Lifestyle', icon: 'lifestyle' }, { id: 'inventory', label: 'Inventario', icon: 'inventory' }, { id: 'investments', label: 'Inversiones', icon: 'investments' }] },
  { label: 'Sistema', items: [{ id: 'settings', label: 'Configuración', icon: 'settings' }] },
];

function NavIcon({ name, active }: { name: keyof typeof symbols; active: boolean }) {
  return <SymbolView name={symbols[name]} size={15} tintColor={active ? Colors.orange : Colors.muted} fallback={<Text style={[styles.navIconFallback, active && styles.navTextActive]}>{name.slice(0, 1).toUpperCase()}</Text>} />;
}
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
    <View style={styles.app}>
      ...
      <View style={styles.main}>
        ...
        {!desktop ? <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.mobileNav} contentContainerStyle={styles.mobileNavContent}>{navItems.map((item) => <Link key={item.id} href={{ pathname: '/[screen]', params: { screen: item.id } }} asChild><Pressable style={StyleSheet.flatten([styles.mobileNavItem, active === item.id ? styles.mobileNavItemActive : undefined])}><NavIcon name={item.icon} active={active === item.id} /><Text style={[styles.mobileNavText, active === item.id && styles.navTextActive]}>{item.label}</Text></Pressable></Link>)}</ScrollView> : null}
...
const styles = StyleSheet.create({
  ...
  navScroll: { paddingVertical: 14, paddingBottom: 26 }, navGroup: { marginBottom: 15 }, navGroupLabel: { color: Colors.muted, fontFamily: Fonts.mono, fontSize: 7, fontWeight: '900', letterSpacing: 1.4, textTransform: 'uppercase', paddingHorizontal: 10, marginBottom: 6, opacity: 0.68 }, navItem: { height: 35, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, borderRadius: Radius.sm, gap: 10, borderLeftWidth: 2, borderLeftColor: 'transparent' }, navItemActive: { backgroundColor: Colors.panelSoft, borderLeftColor: Colors.orange }, navIconFallback: { color: Colors.muted, fontFamily: Fonts.mono, fontSize: 9, width: 15, height: 15, textAlign: 'center' }, navText: { color: Colors.textSoft, fontFamily: Fonts.sans, fontSize: 10, fontWeight: '600' }, navTextActive: { color: Colors.text }, navDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: Colors.orange, marginLeft: 'auto' },
  ...
  mobileNav: { flexGrow: 0, borderBottomWidth: 1, borderBottomColor: Colors.line, backgroundColor: Colors.bgRaised }, mobileNavContent: { paddingHorizontal: 12, paddingVertical: 8, gap: 6 }, mobileNavItem: { minHeight: 38, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 11, paddingVertical: 8, borderWidth: 1, borderColor: Colors.line, borderRadius: Radius.sm }, mobileNavItemActive: { borderColor: Colors.orange, backgroundColor: Colors.orangeSoft }, mobileNavText: { color: Colors.muted, fontFamily: Fonts.mono, fontSize: 9, fontWeight: '800' },
