...
const groups: { label: string; items: { id: ScreenId; label: string; icon: string }[] }[] = [
  ...
  { label: 'Phase 2', items: [{ id: 'analytics', label: 'Career Analytics', icon: '⌁' }, { id: 'records', label: 'Récords', icon: '↑' }, { id: 'trophy-room', label: 'Trophy Room', icon: '▲' }, { id: 'lifestyle', label: 'Lifestyle', icon: '$' }, { id: 'inventory', label: 'Inventario', icon: '□' }, { id: 'investments', label: 'Inversiones', icon: '%' }] },
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
    <View style={styles.app}>
      ...
      <View style={styles.main}>
        ...
        <ScrollView style={styles.contentScroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>{editorial ? <GameImage asset={editorial.asset} ratio="cinema" style={styles.editorial}><Text style={styles.editorialLabel}>{editorial.label}</Text></GameImage> : null}{children}</ScrollView>
      </View>
    </View>
  </SafeAreaView>;
}
