...
export const CAREER_EVENTS: CareerDecision[] = BLUEPRINTS.flatMap((blueprint, blueprintIndex) => contexts.map((context, variant) => ({
  ...
  choices: [
    ...
    { id: 'risk', title: blueprint.risky, description: 'Tomás una postura fuerte y aceptás sus posibles consecuencias.', risk: 'high', effectPreview: '+impacto · consecuencias variables', effects: { confidence: 5, chemistry: -5, reputation: variant % 2 ? -3 : 5, fatigue: 7, burnout: 4, attributes: { aim: 1, discipline: -1 }, flags: { [`${blueprint.category}Risk`]: true } }, outcome: variant % 2 ? 'La apuesta genera tensión y obliga a demostrar resultados.' : 'La jugada sale bien y tu nombre empieza a circular.' },
  ],
})));
