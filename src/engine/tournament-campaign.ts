...
function opponentFor(state: CareerState, tournament: Tournament, faced: string[]) {
  ...
  return pick(rngFor(state.careerSeed, tournament.id, state.season, state.matches.length, faced.length), close.length ? close : regional.length ? regional : TEAMS.filter((candidate) => candidate.id !== team.id && !faced.includes(candidate.id)));
}
...
export function simulateTournamentCampaign(source: CareerState, tournament: Tournament) {
  ...
  next.player.money += Math.round(prizeMoney * next.contract.prizeShare);
  next.player.reputation = clamp(next.player.reputation + (champion ? tournament.reputationImpact : deepRun ? Math.max(2, Math.round(tournament.reputationImpact / 3)) : wins ? 1 : -1));
