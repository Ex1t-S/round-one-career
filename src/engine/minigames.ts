...
import { ACTIVE_MAP_IDS } from '@/data/maps';
...
export const minigameStepCount = (id: MinigameId) => ['clutch', 'utility-memory', 'spray-control', 'overtime-decision'].includes(id) ? 3 : id === 'map-veto' ? 2 : 1;
...
export function createMinigame(state: CareerState, definitionId: MinigameId, matchContext: string, majorCampaignId?: string): MinigameState {
  const id = `mini-${state.season}-${state.minigameHistory.length + 1}`;
  const definition = getMinigameDefinition(definitionId);
  const random = rngFor(id, matchContext, definitionId);
  const opponentId = state.pendingMatchId?.split('|')[1];
  const ownTeam = getTeam(state.teamId); const opponent = opponentId ? getTeam(opponentId) : undefined;
  let options = [...definition.options]; let promptSequence: string[] = []; let clue = definition.description; const contextMetrics: Record<string, number | string> = {};
  if (definitionId === 'map-veto') { options = [...ACTIVE_MAP_IDS]; promptSequence = [opponent?.mapPool[0] ?? options[0], ownTeam.mapPool[0] ?? options[1]]; clue = `Primero baneá la fortaleza de ${opponent?.name ?? 'tu rival'}; después elegí un mapa fuerte propio.`; }
  if (definitionId === 'utility-memory') promptSequence = Array.from({ length: 3 }, () => pick(random, options));
  if (definitionId === 'spray-control') { options = ['pull-down', 'counter-left', 'counter-right', 'burst-reset']; promptSequence = Array.from({ length: 3 }, () => pick(random, options)); }
  if (definitionId === 'peek-timing') promptSequence = ['perfect'];
  if (definitionId === 'minimap-read') { clue = `Vistos por segundos: ${pick(random, ['dos en A, uno en medio', 'utility en B y silencio en A', 'AWP en largo y rotación temprana'])}. Elegí rotate, stack, fake o hold.`; contextMetrics.spotted = 3 + Math.floor(random() * 3); }
  if (definitionId === 'round-buy') Object.assign(contextMetrics, { money: 1900 + Math.floor(random() * 3600), lossBonus: 1400 + Math.floor(random() * 4) * 500, teamWeapons: Math.floor(random() * 5) });
  if (definitionId === 'retake-save') Object.assign(contextMetrics, { alive: 1 + Math.floor(random() * 4), enemies: 1 + Math.floor(random() * 4), utility: Math.floor(random() * 4), seconds: 12 + Math.floor(random() * 23) });
  if (definitionId === 'clutch') { const enemies = 1 + Math.floor(random() * 5); contextMetrics.enemies = enemies; clue = `1v${enemies}: escuchás un paso corto y la bomba está sin plantar.`; }
  if (definitionId === 'overtime-decision') Object.assign(contextMetrics, { ctMoney: 10000, tMoney: 10000, rounds: 6 });
  return { id, definitionId, majorCampaignId, matchContext, difficulty: state.settings.minigameDifficulty, step: 0, choices: [], options, promptSequence, clue, contextMetrics, score: 0, startedAt: new Date().toISOString() };
}

function expectedChoices(game: MinigameState, count: number) {
  const definition = getMinigameDefinition(game.definitionId);
  if (game.promptSequence.length) return game.promptSequence.slice(0, count);
  const random = rngFor(game.id, game.matchContext, game.difficulty);
  return Array.from({ length: count }, () => pick(random, game.options.length ? game.options : definition.options));
}

export function resolveMinigame(state: CareerState, game: MinigameState, choices: string[], simulated = false): MinigameResult {
  const definition = getMinigameDefinition(game.definitionId);
  const steps = minigameStepCount(game.definitionId);
...
export function autoSimulateMinigame(state: CareerState, definitionId: MinigameId, matchContext: string, majorCampaignId?: string) {
  ...
  const steps = minigameStepCount(definitionId);
  const random = rngFor(game.id, 'auto');
  const choices = Array.from({ length: steps }, () => pick(random, game.options));
